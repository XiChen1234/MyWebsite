...
const skills = {
  languages: [
    ...
    { name: 'C', level: 1, description: '包含C/C++/C#，具备语言基础，了解其特性和适用场景，具备快速上手的能力' }
  ],
  backend: [
    ...
    { name: 'RabbitMQ', level: 2, description: '了解其作为消息队列的基本应用，如实现异步处理' }
  ],
  frontend: [
  ...
  tools: [
    ...
    { name: 'Postman', level: 2, description: '了解其作为API测试工具的基本使用' }
  ]
};
...
<template>
  <div class="skill-page" id="skill">
    ...
    <div class="content">
      <SkillSection v-for="(sectionSkills, sectionKey) in skills" :key="sectionKey" :section="sectionKey"
        :title="sectionTitles[sectionKey as keyof typeof sectionTitles]" :skills="sectionSkills"
        :is-expanded="expandedSection === sectionKey" :on-toggle="handleSectionClick" />
    </div>
  </div>
</template>
